...
Every function named here is re-exported at the package root, so
`att.calculate_assembly_index` and
`assemblytheorytools.assembly.calculate_assembly_index` are the same function.
Result types such as {class}`~assemblytheorytools.assembly.RustSearchResult` are
imported from the submodule that defines them.

## Start from what you have

| You have | Start with | Guide |
| --- | --- | --- |
| A SMILES string | {func}`~assemblytheorytools.tools_graph.smi_to_nx`, then {func}`~assemblytheorytools.assembly.calculate_assembly_index` | {doc}`guide/molecules` |
| An RDKit `Mol` | {func}`~assemblytheorytools.assembly.calculate_assembly_index` directly — no conversion needed | {doc}`guide/molecules` |
...
| {term}`Path` | Third return value of the calculations above | — | {class}`~networkx.DiGraph`; nodes are the virtual objects and the joining steps, each carrying its object in a `vo` attribute | Plotting with {func}`~assemblytheorytools.tools_plotting.plot_pathway`, levelling, comparing routes |
...
| Assembly depth of one pathway | {func}`~assemblytheorytools.construction.assign_levels` | Topologically ordered pathway | `None`; the graph passed in gains a `level` attribute on every node (`inplace=False` returns an annotated copy instead) | Layer-by-layer inspection and layout; this is the depth of *that* pathway, not the object's minimum |
...
| Assembly $A$ from known indices | {func}`~assemblytheorytools.assembly.calculate_assembly_from_indices` | Assembly indices and their copy numbers | `float` | The same equation when the indices are already in hand, composed from parts or measured |
| {term}`Copy numbers <Copy number>` $n_i$ | {func}`~assemblytheorytools.assembly.count_copies` | Observed objects, with repeats | The unique objects and how often each was seen | Turning a list of observations into the counted input the assembly equation needs |
| {term}`Joint assembly space` | {func}`~assemblytheorytools.assembly.joint_assembly_space` | One minimum pathway per object | {class}`~networkx.DiGraph` | The union approximation: observed objects plus the contingent intermediates that build them |
| {term}`Exploration ratio` | {func}`~assemblytheorytools.assembly.exploration_ratio` | The same pathways | `float`, observed ÷ total | How fully an ensemble samples its own joint assembly space; low means directed exploration |
...
| Many indices at once | {func}`~assemblytheorytools.assembly.calculate_assembly_index_parallel` | List of graphs plus a settings dictionary (required; pass `None` for the defaults) | Lists of indices, virtual objects and pathways | Datasets and sweeps; see {doc}`guide/parallel` |
...
| Quantity | Function | Input | Output | Use it for |
| --- | --- | --- | --- | --- |
| Recursive MA | {class}`~assemblytheorytools.recursive_ma.MAEstimator` and {func}`~assemblytheorytools.recursive_ma.rma_estimate_ma` | Fragmentation tree and a molecular weight | {class}`~numpy.ndarray` of Monte Carlo samples from {meth}`~assemblytheorytools.recursive_ma.MAEstimator.estimate_MA`; {func}`~assemblytheorytools.recursive_ma.rma_estimate_ma` returns their mean as a `float` | Estimating MA from tandem MS for an unidentified sample; call the estimator when the spread is wanted |
...
| Assembly index from IR peaks | {func}`~assemblytheorytools.tools_data.estimate_ai_from_ir_peaks` | Peak counts, reference indices, a model function and a starting parameter guess | Fitted parameters and predicted indices | Calibrating and applying the IR correlation |
...
| Quantity | Function | Input | Output | Use it for |
| --- | --- | --- | --- | --- |
| Graph-theoretic indices | {func}`~assemblytheorytools.complexity_scores.bertz_complexity`, {func}`~assemblytheorytools.complexity_scores.bottcher`, {func}`~assemblytheorytools.complexity_scores.wiener_index`, {func}`~assemblytheorytools.complexity_scores.balaban_index`, {func}`~assemblytheorytools.complexity_scores.randic_index`, {func}`~assemblytheorytools.complexity_scores.kirchhoff_index` | RDKit `Mol` | `float`, except `int` for {func}`~assemblytheorytools.complexity_scores.wiener_index` | Benchmarking assembly index against established topological measures |
| Shape and substructure | {func}`~assemblytheorytools.complexity_scores.spacial_score`, {func}`~assemblytheorytools.complexity_scores.proudfoot`, {func}`~assemblytheorytools.complexity_scores.mc1`, {func}`~assemblytheorytools.complexity_scores.mc2` | RDKit `Mol` | `float`, except `int` for {func}`~assemblytheorytools.complexity_scores.mc2`, which is a count | Complexity that topology alone misses |
| Compression proxies | {func}`~assemblytheorytools.complexity_scores.compression_zlib_smi`, {func}`~assemblytheorytools.complexity_scores.compression_ratio_zlib_graph`, {func}`~assemblytheorytools.complexity_scores.shannon_entropy` | RDKit `Mol`, NetworkX graph, or a string | `int` compressed length for {func}`~assemblytheorytools.complexity_scores.compression_zlib_smi`, `float` for the other two | A crude redundancy measure to contrast with reuse-based ones |
...
| Reaction-template products | {func}`~assemblytheorytools.reassembler.assemble` and {func}`~assemblytheorytools.reassembler.origami` | Two RDKit `Mol` objects plus `sites` (1 or 2) for `assemble`; one `Mol` for `origami` | One randomly chosen product, or `None` when no template applies; `origami` returns a list of ring-closed products, or the input unchanged when none exists | Chemically plausible expansion; see {doc}`guide/reassembly` |
| Merged assembly space of several molecules | {class}`~assemblytheorytools.reassembler.MoleculeSpace` | {class}`~assemblytheorytools.reassembler.Molecule` objects with reconstructed pathways | One joined assembly graph, with shared intermediates merged | Seeing what a set of molecules shares |
| Generated molecule space | {class}`~assemblytheorytools.reassembler.MoleculeGenerationAssemblyPool` | A {class}`~assemblytheorytools.reassembler.MoleculeSpace` to draw fragments from | Generated molecules, keyed by the number of assembly steps taken | Multi-step reassembly runs |
