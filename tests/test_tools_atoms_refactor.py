"""Compatibility checks for atom calculators without external executables."""
...
def test_orca_truthy_solvent_and_dispersion_options_keep_legacy_defaults(
    tmp_path, option
):
    calc = atoms_tools.orca_calc_preset(
...
def test_orca_qmmm_keeps_legacy_blocks(tmp_path):
    calc = atoms_tools.orca_calc_preset(
        orca_path="/opt/orca",
        directory=tmp_path,
        calc_type="QM/XTB2",
