...
# def invert_undir_str_molecule(graph: nx.Graph, edge_color_dict: dict[str, str]) -> str:
#         """
#         Reconstruct the original undirected string from the graph and edge_color_dict.

#         Args:
#             graph (nx.Graph): The molecular graph generated by get_undir_str_molecule.
#             edge_color_dict (dict[str, str]): Mapping from character to edge color string.

#         Returns:
#             str: The original undirected string.
#         """
#         # Build inverse mapping: color string -> character
#         color_to_char = {v: k for k, v in edge_color_dict.items()}
#         # Sort edges by node order: (0,1), (1,2), ..., (n-1, n)
#         edges = [(u, v, d) for u, v, d in graph.edges(data=True)]
#         edges_sorted = sorted(edges, key=lambda x: min(x[0], x[1]))
#         # Reconstruct string by mapping edge color to character
#         result = ""
#         for u, v, d in edges_sorted:
#             color_int = d["color"]
#             color_str = str(color_int)
#             char = color_to_char[color_str]
#             result += char
#         return result
